...
* 参考链接：[https://www.cnblogs.com/0YHT0/p/3403474.html](https://www.cnblogs.com/0YHT0/p/3403474.html)。大致总结为：**我们的数据是存在内存中的，而每个cpu对应的存储方式是不一致的**。所谓**大端模式就是高位存在内存低位上**，eg：假设要存储12345678这个数字时，两两为一对。87属于第一位、56属于第二位.....以此类推。那么，我们就能知道12是最高位，所以它会被存到内存的低位。拿上述链接的总结来说就是如下表所示：
...
* 运行结果如下：
  ...
  ![证明调用wait方法升级为重量锁.png](证明调用wait方法升级为重量锁.png)
